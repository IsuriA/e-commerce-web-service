...
{
    public class PaymentToPaymentDtoConverter : ITypeConverter<Payment, PaymentDto>
    {
        public PaymentDto Convert(Payment source, PaymentDto destination, ResolutionContext context)
        {
            if (destination == null)
            {
                destination = new PaymentDto();
            }
            if (source == null)
            {
                return destination;
